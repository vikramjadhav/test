...
/*
 *    Function     :     warnx
 *    Parameters    :    1. print_error     :     flag for printing error msg.
 *                    2. fmt               :    format string
 *    Return value:    none
 */
 ...
 * Description  :   Exchange the block from nonopt_start to nonopt_end
 *                  with the block from nonopt_end to opt_end (keeping
 *                  the same order of arguments in each block).
 */
 ...
 * Description  :   Parse argc/argv argument vector.  Called by user
 *                  level routines. Returns -2 if -- is found (can
 *                  be long option or end of options marker).
 */
...
start:
    ...
    if (optchar == 'W' && oli[1] == ';') {          /* -W long-option */
        /*  what if no long options provided (called by getopt)? */
        if (*place)
            return -2;
    ...
    if (*++oli != ':') {                    /* doesn't take argument */
        ...
        if (*place)                     /* no white space */
            optarg = place;
        /*  disable test for :: if PC? (GNU doesn't) */
...
/*
 * Function     :   getopt
 * Description  :   Parse argc/argv argument vector.
 *
...
#endif
                /*
                 *  GNU sets optopt to val regardless of
                 * flag
                 */
...
#endif
                /*
                 * GNU sets optopt to val regardless
                 * of flag
                 */
                if (long_options[match].flag == NULL)
                    optopt = long_options[match].val;
