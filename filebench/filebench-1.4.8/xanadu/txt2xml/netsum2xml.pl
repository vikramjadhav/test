...
#Usage:  netsum2xml_mf.pl <infile1> <runid>
...
sub one_xml {
# made $infile (full path to stat/input file) global - modified by duc
  our ($infile, $runid) = @_; 
...
# if-else statement - handles both netsum and davenet by figuring
# out what the name of the input file is and printing it accordingly
# written by duc
  if ($infile =~ /netsum.*/) {
    xml_start_stat_doc(name => "\"Netsum\"");
  } else {
    xml_start_stat_doc(name => "\"Davenet\"");
  }

# commented out by duc due to the above
  #xml_start_stat_doc(name => "\"Davenet\"");
